"use client";

import Link from "next/link";
import { useEffect, useState } from "react";
import { usePathname } from "next/navigation";

type ConsentState = "accepted" | "rejected" | null;

export default function CookieBanner() {
  const pathname = usePathname();
  const isDE = pathname.startsWith("/de");
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    const consent = localStorage.getItem(
      "treilix-cookie-consent"
    ) as ConsentState;

    if (!consent) {
      setVisible(true);
    }
  }, []);

  const acceptCookies = () => {
    localStorage.setItem("treilix-cookie-consent", "accepted");
    window.dispatchEvent(new Event("treilix-cookie-consent-changed"));
    setVisible(false);
  };

  const rejectCookies = () => {
    localStorage.setItem("treilix-cookie-consent", "rejected");
    window.dispatchEvent(new Event("treilix-cookie-consent-changed"));
    setVisible(false);
  };

  if (!visible) return null;

  return (
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white/95 p-4 shadow-2xl backdrop-blur">
      <div className="mx-auto flex max-w-6xl flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="text-sm text-slate-700">
          <p className="font-semibold text-slate-900">
            {isDE ? "Wir verwenden Cookies" : "We use cookies"}
          </p>
          <p className="mt-1 leading-6">
            {isDE ? (
              <>
                Wir verwenden notwendige Cookies, damit Treilix ordnungsgemäß
                funktioniert. Mit Ihrer Zustimmung verwenden wir auch
                Analyse-Cookies, um die Nutzung der Website zu verstehen und
                die Plattform zu verbessern. Lesen Sie unsere{" "}
                <Link href="/de/privacy" className="text-blue-600 hover:underline">
                  Datenschutzerklärung
                </Link>
                ,{" "}
                <Link href="/de/terms" className="text-blue-600 hover:underline">
                  Nutzungsbedingungen
                </Link>{" "}
                und{" "}
                <Link href="/de/imprint" className="text-blue-600 hover:underline">
                  Impressum
                </Link>
                .
              </>
            ) : (
              <>
                We use essential cookies to keep Treilix working properly. With
                your consent, we also use analytics cookies to understand
                website usage and improve the platform. Read our{" "}
                <Link href="/privacy" className="text-blue-600 hover:underline">
                  Privacy Policy
                </Link>
                ,{" "}
                <Link href="/terms" className="text-blue-600 hover:underline">
                  Terms
                </Link>{" "}
                and{" "}
                <Link href="/imprint" className="text-blue-600 hover:underline">
                  Imprint
                </Link>
                .
              </>
            )}
          </p>
        </div>

        <div className="flex flex-col gap-2 sm:flex-row">
          <button
            onClick={rejectCookies}
            className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 transition-colors duration-150 hover:bg-slate-50"
          >
            {isDE ? "Ablehnen" : "Reject"}
          </button>

          <button
            onClick={acceptCookies}
            className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors duration-200 hover:bg-blue-700"
          >
            {isDE ? "Akzeptieren" : "Accept"}
          </button>
        </div>
      </div>
    </div>
  );
}